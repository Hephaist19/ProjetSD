    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < numProcess; i++){
            ...
            do{
                ...
                tire=true;
                System.out.print("On a tiré "+alea+". ");

                for(int k=0;k<i;k++){ //on vérifie dans le tableau dejaTire 
                    if(alea==dejaTire[k]){
                        tire=false;
                        System.out.println("Déjà tiré, on re-tire");
                        break;
                    }
                }
            }while(!tire); //on vérifie qu'on ne l'a pas déjà tiré
            //Si le chiffre tiré est déjà présent dans le tableau alors on retire à nouveau jusqu'à avoir un chiffre non tiré
